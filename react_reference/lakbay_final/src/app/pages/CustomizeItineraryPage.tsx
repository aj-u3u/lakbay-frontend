...
export function CustomizeItineraryPage() {
  const navigate = useNavigate();
  const location = useLocation();
  const state = location.state as CustomizeState | undefined;

  const [itinerary, setItinerary] = useState<ItineraryDay[]>(state?.itinerary || []);
  const [costBreakdown, setCostBreakdown] = useState<CostItem[]>(state?.costBreakdown || []);
  ...
  const handleSaveTrip = () => {
    // Navigate back to AI chat with updated itinerary
    navigate('/ai-planner', {
      state: {
        returnFromCustomize: true,
        isSolo: state?.isSolo,
  ...
  return (
    <div className="min-h-screen bg-background pb-6">
      {/* Header */}
      <div className={`${isGroupTrip ? 'bg-secondary' : 'bg-primary'} text-white p-6 flex items-center gap-4 shadow-lg`}>
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="flex-1">
          ...
          <p className="text-white/80 text-sm">{state.planTitle}</p>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          {itinerary.map((day, dayIndex) => (
            <div key={day.day} className="bg-card rounded-2xl p-5 shadow-sm">
              {/* Day Header */}
              <div className="mb-4">
